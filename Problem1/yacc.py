...
def division(l):
    ...
    for i in range (len(l)-1):
        if isinstance(l[i+1], int):
            if l[i+1] == 0:
                return 'Dinominator cannot be zero'
            else:
                div = div / l[i+1]
        else:
            if lett[l[i+1]] == 0:
...
def p_exp_let(p):
    'exp : LPAREN LET letexp call RPAREN'
    for key in lett:
        del lett[key]
    p[0] = p[4]
...
def p_letexp(p):
    'letexp : LPAREN SIMB NUM RPAREN'
    if DEBUG: print "Assigning", p[3], "to", p[2]
    lett[p[2]] = p[3]
